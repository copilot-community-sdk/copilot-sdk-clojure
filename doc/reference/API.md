...
| `:capi` | map | CAPI (Copilot API) session options. Optional keys: `:enable-web-socket-responses` (boolean) and `:auto-tier` (`:efficiency`, `:balance`, `:intelligence`, or the integrator-only latency preset `:fast`). The tier is serialized as `capi.autoTier`; omission uses or restores the runtime's persisted preference. Supplying a different tier while resuming a resident session requests a safe runtime switch. Experimental live setters, nullable reset, recommendation events, and tier-status APIs are not exposed. ([upstream PR #2437](https://github.com/github/copilot-sdk/pull/2437), [upstream PR #2514](https://github.com/github/copilot-sdk/pull/2514), [upstream PR #2609](https://github.com/github/copilot-sdk/pull/2609)) |
